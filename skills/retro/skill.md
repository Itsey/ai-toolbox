...
For each Behavioural or Guardrail item, determine where it belongs. Load all of the relevant prompt, instructions, skills and agent files that have contributed to the session and identify the most suitable location to add a correction.
...
**Specificity rule**: always store in the most specific location that applies. A lesson about the capture skill goes in `capture/skill.md`, not in global copilot-instructions.md.

## Phase 4: Present Findings

Present all findings in a single table for review for example:

| #    | Signal                                       | Category    | Proposed rule                                 | Destination                    |
| ---- | -------------------------------------------- | ----------- | --------------------------------------------- | ------------------------------ |
| 1    | AI used `list` instead of `ls` for gog drive | Guardrail   | `gog drive` subcommand is `ls` not `list`     | Global copilot-instructions.md |
...
Good: "After changes always check for passing unit tests"
